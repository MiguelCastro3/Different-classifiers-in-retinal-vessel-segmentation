...
def geral(normalizacao, kernel, pasta):  
    ficheiro = open('Resultados/Pontos aleatórios/' + normalizacao + '/' + kernel + '/' + pasta + '/métricas_' + normalizacao + '_' + kernel + '_' + pasta + '.txt','w') #cria um ficheiro para guardar as métricas cálculadas
    ...
    features_training = joblib.load('Features/Pontos aleatórios/features_training') #carrega as features das imagens test, uma a uma
    labels_training  = joblib.load('Features/Pontos aleatórios/labels_training') #carrega as labels das imagens test, uma a uma  
    classificador = joblib.load('Features/Pontos aleatórios/classificador_' + normalizacao + '_' + kernel) #carrega o classificador
    scaler_training = joblib.load('Features/Pontos aleatórios/scaler_' + kernel)
    scaler_test = joblib.load('Features/Pontos aleatórios/scaler_' + kernel)
    ...
    for i in range(inicio_contagem, inicio_contagem + 20):
        ...
        scipy.misc.toimage(imagem_reconstruida).save('Resultados/Pontos aleatórios/' + normalizacao + '/' + kernel + '/' + pasta + '/' + numero + '_imagem_reconstruida.png')
    ...
    ROC.savefig('Resultados/Pontos aleatórios/' + normalizacao + '/' + kernel + '/' + pasta + '/ROC_' + normalizacao + '_' + kernel + '_' + pasta + '.png')
    
    return


if __name__ == '__main__':
    geral('global', 'linear', 'test')
    geral('global', 'linear', 'training')
    geral('global', 'rbf', 'test')
    geral('global', 'rbf', 'training')
    geral('individual', 'linear', 'test')
    geral('individual', 'linear', 'training')
    geral('individual', 'rbf', 'test')
    geral('individual', 'rbf', 'training')
