...
def geral(vasos, background, normalizacao, kernel, pasta):  
    ficheiro = open('Resultados/Vasos e background/' + vasos + '% vasos e ' + background + '% background/' + normalizacao + '/' + kernel + '/' + pasta + '/métricas_' + normalizacao + '_' + kernel + '_' + pasta + '.txt','w') #cria um ficheiro para guardar as métricas cálculadas
    ...
    features_training = joblib.load('Features/Vasos e background/' + vasos + '% vasos e ' + background + '% background/features_training') #carrega as features das imagens test, uma a uma
    labels_training  = joblib.load('Features/Vasos e background/' + vasos + '% vasos e ' + background + '% background/labels_training') #carrega as labels das imagens test, uma a uma  
    classificador = joblib.load('Features/Vasos e background/' + vasos + '% vasos e ' + background + '% background/classificador_' + normalizacao + '_' + kernel) #carrega o classificador
    scaler_training = joblib.load('Features/Vasos e background/' + vasos + '% vasos e ' + background + '% background/scaler_' + kernel)
    scaler_test = joblib.load('Features/Vasos e background/' + vasos + '% vasos e ' + background + '% background/scaler_' + kernel)
    ...
    for i in range(inicio_contagem, inicio_contagem + 20):
        ...
        scipy.misc.toimage(imagem_reconstruida).save('Resultados/Vasos e background/' + vasos + '% vasos e ' + background + '% background/' + normalizacao + '/' + kernel + '/' + pasta + '/' + numero + '_imagem_reconstruida.png')
    ...
    ROC.savefig('Resultados/Vasos e background/' + vasos + '% vasos e ' + background + '% background/' + normalizacao + '/' + kernel + '/' + pasta + '/ROC_' + normalizacao + '_' + kernel + '_' + pasta + '.png')
    print("") #parágrafo
    print("") #parágrafo
    print("") #parágrafo
...
percentagem_vasos = ['5', '10', '15', '20', '50', '80']
percentagem_background = ['95', '90', '85', '80', '50', '20']
...
if __name__ == '__main__':
    for i in range(len(percentagem_vasos)):
        geral(percentagem_vasos[i], percentagem_background[i], 'global', 'linear', 'test')
        geral(percentagem_vasos[i], percentagem_background[i], 'global', 'linear', 'training')
        geral(percentagem_vasos[i], percentagem_background[i], 'global', 'rbf', 'test')
        geral(percentagem_vasos[i], percentagem_background[i], 'global', 'rbf', 'training')
        geral(percentagem_vasos[i], percentagem_background[i], 'individual', 'linear', 'test')
        geral(percentagem_vasos[i], percentagem_background[i], 'individual', 'linear', 'training')
        geral(percentagem_vasos[i], percentagem_background[i], 'individual', 'rbf', 'test')
        geral(percentagem_vasos[i], percentagem_background[i], 'individual', 'rbf', 'training')
